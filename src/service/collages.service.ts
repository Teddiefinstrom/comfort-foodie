import { doc, setDoc, deleteDoc, getDocs, serverTimestamp } from "firebase/firestore";
import { subCol } from "./collections.service";
import type { Collage, RecipeLikeData } from "../types/recipe";

export const collagesCol = (uid: string) => 
    subCol("users", uid, "collages");
    ...
    export const addRecipeToCollage = async (uid: string, collageId: string, recipe: RecipeLikeData) => {
        const ref = doc(
    ...
    export const removeRecipeFromCollage = async (uid: string, collageId: string, idMeal: string) => {
        const ref = doc(
            subCol("users", uid, "collages", collageId, "recipes"),
            idMeal
        );
